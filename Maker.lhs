...
<todo>Describe this better.</todo>
...
<todo>Explain the fee unit.</todo>
...
>   _ink  :: Wad,         -- Amount of locked collateral
...
> data Ilk = Ilk {
>   _gem :: Id Tag, -- Collateral token identifier
>   _chi :: Ray,    -- Value of fee unit (in dai)
>   _rum :: Wad,    -- Total dai (in fee unit)
>   _hat :: Wad,    -- Maximum dai (issuance ceiling)
>   _mat :: Ray,    -- Liquidation ratio (collateral value per dai value)
>   _axe :: Ray,    -- Liquidation penalty (fraction of dai)
>   _tax :: Ray,    -- Fee (per-second fraction of dai)
>   _lax :: Sec,    -- Grace period of price feed unavailability
>   _rho :: Sec     -- Timestamp of latest fee unit adjustment
> } deriving (Eq, Show)
...
<p>The <code>Vat</code> record aggregates the records of tokens, CDPs,
CDP types, and price feeds, along with the data of the
feedback mechanism.

> data Vat = Vat {
>   _tags  :: Map (Id Tag) Tag, -- Collateral price feeds
...
> data System =  System {
>   _balances  :: Map (Actor, Token) Wad,  -- Token balances
...
>   | Worry  -- Collateral price feed in limbo
...
liquidation; see Table&nbsp;\ref{table:stages}.
...
>   id_ilk  <- look (vat . urns . ix id_urn . ilk)
>   id_tag  <- look (vat . ilks . ix id_ilk . gem)
>
...
amount of collateral, as long as this would not take the urn below its
liquidation ratio.
...
<p>An urn owner can use <code>shut</code> to close their account, if
the price feed is up to date and the urn is not in liquidation.
...
to receive both the urn's collateral
and the anticoins
...
> 
>  -- Nullify urn's collateral and anticoin quantities
...
<p>When the settler has finished the liquidation of an urn, it
invokes <code>plop</code> to give back any collateral it did not need
to sell and restore the urn.
...
>
>  -- Record the excess collateral as belonging to the urn
...
(type: <code>Sec</code>) is the timestamp of its latest fee unit
adjustment.
